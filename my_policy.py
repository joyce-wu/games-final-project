        ...
        for card in cards:
            ...
            if score is not None:
                ...
                if rank_count[card.rank()] >= 2 and history.total_points() + 3 * card_value <= self._game.pegging_limit():
                    score += 0.5
                # if history.is_start_round() and card_value == 5 and rank_count[card.rank()] == 1:
                #     score -= 0.25
                ...
                # complement = 15 - history.total_points() - card_value
                # if complement > 0:
                #     if any(self._game.rank_value(c.rank()) == complement and c != card for c in cards):
                #         score += 0.25
